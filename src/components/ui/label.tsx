...
import * as LabelPrimitive from "@radix-ui/react-label"
import { cva, type VariantProps } from "class-variance-authority"
import { AlertCircle, CheckCircle, HelpCircle, Crown, Users, Code, Zap } from "lucide-react"
import { cn } from "@/lib/utils"

const labelVariants = cva(
  "leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 transition-colors select-none",
  {
    variants: {
      variant: {
        default: "text-gray-200 font-medium",
        muted: "text-gray-400 font-normal hover:text-gray-300",
        ...
        ghost: "text-gray-400 font-normal hover:text-gray-200 transition-colors",
      },
      size: {
        xs: "text-xs",
        sm: "text-sm",
      ...
      >
        <span className="flex items-center gap-2 relative">
          ...
          <span className="flex items-center gap-1">
            ...
            {tooltip && (
              <span 
                className="text-gray-500 hover:text-gray-300 cursor-help ml-1 transition-colors"
                title={tooltip}
                aria-label="More information"
              >
                <HelpCircle className="h-3 w-3" />
          ...
          {/* Badge */}
          {badge && (
            <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-900/30 text-blue-300 border border-blue-700">
              {badge}
            </span>
          )}
        </span>
...
>(({ language, showIcon = true, children, ...props }, ref) => {
  ...
  return (
    <Label
      ref={ref}
      variant="info"
      icon={showIcon ? <span className="text-xs font-bold">{getLanguageIcon(language)}</span> : undefined}
      badge={language.toUpperCase()}
      className="font-mono"
      {...props}
    >

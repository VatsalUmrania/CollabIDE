    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      ...
      >
        <CardHeader className="pb-4 text-center">
          ...
          <div className={cn(
            "mx-auto flex items-center justify-center w-20 h-20 rounded-full mb-6",
            reasonConfig.bgColor,
            "border-2",
            reasonConfig.borderColor
          )}>
          ...
          {sessionTitle && (
            <p className="text-sm text-gray-400 truncate max-w-[250px] mx-auto">
              {sessionTitle}
            </p>
          )}
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Connection Lost Alert */}
          {sessionEndedReason === 'connection_lost' && (
            <Alert className={cn("border", reasonConfig.borderColor)}>
              <AlertTriangle className={cn("h-4 w-4", reasonConfig.iconColor)} />
              <AlertDescription className="text-gray-300">
                <strong className="text-white">Connection Issue:</strong> If this was unexpected, 
                you can try refreshing the page to reconnect to the session.
              </AlertDescription>
            </Alert>
          )}
          ...
          {/* Next Steps */}
          <div className="space-y-3">
            <h4 className="text-sm font-semibold text-white">What's next?</h4>
            <div className="space-y-2 text-sm text-gray-400">
              {reasonConfig.canRetry && (
                <div className="flex items-start space-x-2">
                  <RefreshCw className="h-4 w-4 mt-0.5 flex-shrink-0 text-blue-400" />
                  <span>Try reconnecting if the issue was temporary</span>
                </div>
              )}
              <div className="flex items-start space-x-2">
                <Home className="h-4 w-4 mt-0.5 flex-shrink-0 text-blue-400" />
                <span>Return to dashboard to view your sessions</span>
              </div>
              <div className="flex items-start space-x-2">
                <StopCircle className="h-4 w-4 mt-0.5 flex-shrink-0 text-green-400" />
                <span>Create a new session for continued collaboration</span>
              </div>
            </div>
          </div>
        ...
        {/* Action Buttons */}
        <div className={cn(
          "flex justify-center p-6 border-t border-gray-700 bg-gray-800/50 rounded-b-lg",
          reasonConfig.canRetry ? "space-x-3" : ""
        )}>
          {reasonConfig.canRetry && (
            <Button 
